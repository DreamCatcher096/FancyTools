...
class_name = {1: '左眉毛上', 2: '左眉毛下', 3: '左眼皮上', 4: '右眉毛上',
              5: '右眉毛下', 6: '右眼皮上', 7: '上嘴唇上', 8: '上嘴唇下'}
path = r"E:\label\img_68_all\img_68_all"
txt_path = r"E:\PycharmProjects\image_names.txt"
save_path = r"E:\label\results"
image_names = []
...
for image in image_names:
    ...
    for cl in range(1, 9):
        ...
        while True:
            gmi = img.copy()
            for p in points:
                cv2.circle(gmi, (p[0], p[1]), 2, colors[p[2] - 1], -1)
    ...
    with open(os.path.join(save_path, name), 'wb') as res:
        res.write("%s\r\n" % image)
        for i in points:
            line = str(i[0] / 0.6) + ' ' + str(i[1] / 0.6) + ' ' + str(i[2])
            res.write("%s\r\n" % line)
